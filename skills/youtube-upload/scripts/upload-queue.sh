...
# Iterate the videos/ folder, upload one video at a time, respect daily cap,
# move failed files to videos/failed/, send email on session expiry.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT="${1:-${YT_UPLOAD_PROJECT:-$HOME/codeplay/ijp_research/CSOS/socials/youtube-upload}}"
...
DAILY_CAP="$(read_yaml daily_cap 50)"
PACE="$(read_yaml pace_seconds 60)"
...
if ! "$SCRIPT_DIR/preflight.sh" "$PROJECT" 2>>"$LOG"; then
  ...
  if (( rc == 12 )); then
    "$SCRIPT_DIR/notify-email.sh" "YouTube session expired" \
      "Sign back in to YouTube Studio in the Chrome window on port 9222, then the queue will resume on next fire." \
      "$NOTIFY_EMAIL"
  fi
  exit "$rc"
fi
  ...
  if url=$(python3 "$SCRIPT_DIR/upload-one.py" "$video" 2>>"$LOG"); then
    ...
    case "$rc" in
      75)  # session expired mid-loop
        "$SCRIPT_DIR/notify-email.sh" "YouTube session expired mid-queue" \
          "Sign back in to YouTube Studio. Queue paused after ${uploaded} successful uploads today." \
          "$NOTIFY_EMAIL"
        exit 75
        ;;
      77)  # YouTube daily limit
        _log "queue: YouTube rate-limited; leaving file in place for tomorrow"
        exit 0
        ;;
      *)
